...
\end{center}
  ...
  Pour cela, créez dans le fichier \cmd{tp2\_fonction.sci} une fonction \mtlb{[k]=IndexInterval(a,b,n,x)}. Cette fonction doit satisfaire les conditions suivantes:
  \begin{itemize}
    \item Étant donnés les bornes \mtlb{a} et \mtlb{b}, le nombre de sous intervalles \mtlb{n} et un vecteur \mtlb{x}, elle doit retourner un vecteur \mtlb{k}, de la même taille que \mtlb{x}, qui contient les indices des intervalles contenant les coordonnées de \mtlb{x}.

    \item Pour des raisons techniques, on demande en plus que si une coordonnée de \mtlb{x} est inférieure à \mtlb{a}, la valeur correspondante rendue doit être \mtlb{1}, et si elle est supérieure à \mtlb{b}, elle doit être \mtlb{n}.
  \end{itemize}
  ...
  \begin{enumerate}
    ...
    \item Une fois la fonction faite pour fonctionner avec un vecteur \mtlb{x}, vérifiez que l'instruction \mtlb{IndexInterval(0,1,3,[-1,0,.5,1,2])} renvoie \mtlb{1.    1.    2.    3.    3.}.
  \end{enumerate}
...
\begin{exo} (Interpolation constante par morceaux)
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      \item Pour cela, créez une fonction \mtlb{[v,t] = InterpConst(a,b,n,f)} qui étant donnée une fonction \mtlb{f} renvoie dans \mtlb{t} les valeurs des $x_{i-\frac12}$ pour $i=1,\ldots,n$, et en \mtlb{v} les valeurs de $f$ en ces points.
      ...
      \item Pour \mtlb{n=2,4,...,2^10} calculez l'erreur d'interpolation (en norme infinie). Puis sur un même graphique représentez (en échelle logarithmique) cette erreur et la majoration théorique de l'erreur.
    \end{enumerate}
  \end{enumerate}
\end{exo}
...
\begin{exo} (Interpolation affine par morceaux)
  ...
  \begin{enumerate}
    ...
    \item On se propose maintenant de construire la fonction $s_1$ affine sur chaque intervalle $I_i$ et qui coïncide avec $f$ aux points $x_i$ et $x_{i+1}$ :
      ...
      \begin{enumerate}
      \item Créez une fonction \mtlb{[v,t] = InterpAffine(a,b,n,f)} qui étant donnée une fonction \mtlb{f} renvoie dans \mtlb{t} les valeur des $x_{i}$ pour $i=0,\ldots,n$, et en \mtlb{v} les valeurs de $f$ en ces points.
...
\begin{exo} (Interpolation par splines)
  ...
  \begin{enumerate}
    ...
    qui détermine les coefficients \mtlb{p} du polynôme $P(x)= p_{1}+p_{2}x+p_{3}x^{2}+p_{4}x^{3}$ qui a pour valeurs \mtlb{y1} en \mtlb{x1} et \mtlb{y2} en \mtlb{x2}, et dont les dérivées dans ces deux points sont respectivement \mtlb{d1} et \mtlb{d2}. Pour vérifier que votre procédure fonctionne, testez-la avec \mtlb{CubicCoeffs(0,1,2,3,4,5)} et le résultat devrait être \mtlb{2. 4. - 10. 7.} (éventuellement en colonne).
    \begin{center}
      \includegraphics[width=9cm]{CubbicCoeffs}
    \end{center}

    \item Écrire la procédure \mtlb{[y] = CubicEval(p,x)} qui, étant donnés les coefficients \mtlb{p} d'un polynôme et un vecteur \mtlb{x}, évalue ce polynôme en les coordonnées de \mtlb{x}. Pour vérifier que votre procédure fonctionne, testez-la avec \mtlb{CubicEval([1:4],[-1:1])} et le résultat devrait être \mtlb{- 2.    1.    10.}.

    \item En utilisant les procédures \mtlb{CubicCoeffs} et \mtlb{CubicEval}, dessinez la base d'Hermite sur $[0,1]$, c'est-à-dire les $4$ polynômes qui ont une seule des valeurs $P(0)$, $P(1)$, $P'(0)$, $P'(1)$ non nulle, égale à $1$.

    \item En utilisant les procédure \mtlb{CubicCoeffs} et \mtlb{CubicEval}, dessinez les polynômes $P$ sur $[0,1]$ tels que $P(0)=0$, $P(1)=1$, $P'(1)=-7$ et $P'(0)$ prend les valeurs entières de $-10$ à $10$.

    \item Il est temps maintenant de créer la procédure qui évalue la spline cubique d'interpolation. Pour cela créez la procédure \mtlb{[y] = CubicSplin(a,b,v,x)} qui a pour entrées:
    \begin{itemize}
      \item Les bornes \mtlb{a} et \mtlb{b} de l'intervalle d'interpolation.

      \item Les valeurs \mtlb{v=[v(1), v(2), ..., v(n+1)]} d'interpolation aux  $n+1$ nœuds équi-distribués.

      \item Les points \mtlb{x} dans lesquels il faut faire l'évaluation.
    \end{itemize}
    Cette procédure doit renvoyer les valeurs \mtlb{y} de la spline naturelle aux points \mtlb{x}.

    \begin{center}
      \includegraphics[width=15cm]{CubbicSpline}
    \end{center}

    La procédure \mtlb{CubicSplin} devra faire dans l'ordre:
    \begin{itemize}
       \item Créez le vecteur \mtlb{t} des $n+1$ nœuds équi-distribués d'interpolation.
       \item En utilisant \mtlb{d = splin(t,v,"natural")}, déterminez les valeurs des dérivées aux nœuds d'interpolation.
       \item En utilisant \mtlb{IndexInterval} de l'exercice \textbf{1}, déterminez les intervalles dans lesquels se trouvent les points \mtlb{x}.
     ...
     \indent \mtlb{CubicSplin(0,1,[0,2,1],[0:.2:1])}\\
     et le résultat devrait être\\
     \indent \mtlb{0.    1.052    1.816    2.016    1.652    1.}

     \item Reprendre la question 2.(b) de l'exercice \textbf{2}, pour représenter la fonction $\sin(x)$ sur $[-5,5]$, ainsi que la spline cubique naturelle qui l'interpole sur $10$ intervalles de longueur constante. Pour cela normalement il suffit de remplacer la fonction \mtlb{AffinePiecewise} par \mtlb{CubicSplin}.

     \item Modifiez la fonction \mtlb{CubicSplin} (ou créez une nouvelle fonction \mtlb{CubicSplinClamped}) qui permet à la place de \mtlb{splin(t,v,"natural")} d'évoquer \mtlb{d = splin(t,v,"clamped",bd)} pour obtenir une interpolation par spline tendue avec des dérivées aux bords \mtlb{bd(1)} en \mtlb{a}, et \mtlb{bd(2)} en \mtlb{b}.\\
     Puis rajouter le graphe du spline cubique tendu d'interpolation de $\sin(x)$ sur le graphique de la question précédente.

     \item Pour \mtlb{n=2,4,...,2^10} calculez l'erreur d'interpolation par splines cubiques naturelles et tendues (en norme infinie). Puis représentez-les sur un même graphique (en échelle logarithmique).
